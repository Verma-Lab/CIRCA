...
@app.post("/api/session-analytics/batch")
async def get_batch_session_analytics(session_ids: list[str] = Body(...), db: Session = Depends(get_db)):
    try:
        # Fetch all analytics for the given session IDs in one query
        analytics = db.query(SessionAnalytics).filter(
            SessionAnalytics.session_id.in_(session_ids)
        ).order_by(SessionAnalytics.session_id, SessionAnalytics.timestamp).all()

        if not analytics:
            return {}

        # Group and process data by session_id
        results = {}
        for entry in analytics:
            ...
            # Populate message data (simplified; full processing as in get_session_analytics)
            message_data = {
                "id": entry.id,
            ...
            results[session_id]["messages"].append(message_data)

            # Aggregate counts
            results[session_id]["sentiment_distribution"][entry.sentiment] = \
                results[session_id]["sentiment_distribution"].get(entry.sentiment, 0) + 1
            results[session_id]["urgency_distribution"][entry.urgency] = \
                results[session_id]["urgency_distribution"].get(entry.urgency, 0) + 1
            results[session_id]["intent_distribution"][entry.intent] = \
                results[session_id]["intent_distribution"].get(entry.intent, 0) + 1
            results[session_id]["topic_distribution"][entry.topic] = \
                results[session_id]["topic_distribution"].get(entry.topic, 0) + 1
            results[session_id]["total_word_count"] += entry.word_count

        # Finalize each session's data
        for session_id, data in results.items():
            data["message_count"] = len(data["messages"])
            data["start_time"] = data["messages"][0]["timestamp"]
            data["end_time"] = data["messages"][-1]["timestamp"]
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch batch analytics: {str(e)}")
