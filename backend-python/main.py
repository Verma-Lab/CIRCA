...
def call_vertex_endpoint(prompt, max_tokens=1000, temperature=0.3):
    """Helper function to call Vertex AI endpoint with robust response handling."""
    try:
        parameters = {
            "max_output_tokens": max_tokens,
            "temperature": temperature,
            "top_k": 1,  # Strict selection for deterministic output
        ...
        if response.predictions and len(response.predictions) > 0:
            ...
            print(f"[DEBUG] Raw Vertex AI response: {raw_response}")
            # Extract content after "Output:" marker if present
            if "Output:" in raw_response:
                return raw_response.split("Output:")[1].strip().strip('```python').strip('```').strip()
            # Strip any JSON or code block markers
            cleaned_response = raw_response.strip()
            if cleaned_response.startswith('"') and cleaned_response.endswith('"'):
                cleaned_response = cleaned_response[1:-1]
            return cleaned_response
        else:
            print("[DEBUG] No response generated by Vertex AI")
            return "No response generated"
    except Exception as e:
        print(f"[ERROR] Error calling Vertex AI endpoint: {str(e)}")
...
async def vector_flow_chat(request: dict):
    ...
    try:
        ...
        onboarding_status_to_send = "in_progress"
        if not missing_fields:
            onboarding_status_to_send = "completed"
        ...
        if not session_data.get("currentNodeId") and not previous_messages:
            starting_node_id, starting_node_doc = get_starting_node(flow_index)
            print(f"[STARTING NODE] {starting_node_id, starting_node_doc}")
            if starting_node_id:
                current_node_id = starting_node_id
